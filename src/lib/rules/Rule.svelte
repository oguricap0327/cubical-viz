<script lang="ts">
  ...
  let { rule, controls, onStep: onStepProp, activeStep = $bindable(0) }: { rule: RuleDefinition; controls?: Snippet; onStep?: (index: number) => void; activeStep?: number } = $props();
    ...
    function animate() {
      ...
      if (rule.update) {
        if (rule.steps) {
          ...
          (window as any)._activeStep = activeStep;
