<script lang="ts">
  ...
  let dottedSpheres: THREE.Mesh[] = [];
  let arcPoints: THREE.Vector3[] = [];

  const rule: RuleDefinition = {
    name: 'Setup: Given e : A ≃ B',
    judgment: `<div class="nd-rule">${kd('\\dfrac{\\Gamma \\vdash A : \\mathcal{U} \\quad \\Gamma \\vdash B : \\mathcal{U} \\quad \\Gamma \\vdash e : A \\simeq B}{\\Gamma \\vdash \\mathrm{ua}(e) : A =_{\\mathcal{U}} B}')}</div>`,
    ...
    setup: (scene: THREE.Scene, camera: THREE.Camera) => {
      ...
      // Faint dotted line (20 small spheres from A to B)
      dottedSpheres = [];
      for (let i = 0; i < 20; i++) {
        ...
        s.position.set(-3 + (6 * i) / 19, 0, 0);
        s.visible = false;
        scene.add(s);
        dottedSpheres.push(s);
      }

      // Labels
      labelA = createTextSprite('A', '#44DDFF');
      labelA.position.set(-3, 1.2, 0);
      scene.add(labelA);

      labelB = createTextSprite('B', '#FF8844');
      labelB.position.set(3, 1.2, 0);
      scene.add(labelB);
    ...
    update: (t: number, elapsed?: number) => {
      ...
      // Step 3: faint dotted line
      for (const s of dottedSpheres) s.visible = step >= 2;
      labelI.visible = step >= 2;

      // Animate dot along arc
      if (step >= 1 && arcPoints.length > 0) {
        const idx = Math.min(
          Math.floor(t * (arcPoints.length - 1)),
          arcPoints.length - 1,
        );
        dot.position.copy(arcPoints[Math.max(0, idx)]);
      }
    },

    cleanup: (scene: THREE.Scene) => {
      scene.remove(sphereA, sphereB, arcLine, dot, labelA, labelB, labelE, labelI);
      for (const s of dottedSpheres) scene.remove(s);
    },
