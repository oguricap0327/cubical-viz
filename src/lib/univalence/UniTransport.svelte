<script lang="ts">
  ...
  const rule: RuleDefinition = {
    ...
    judgment: `
      <div class="nd-rule">
        <div class="nd-conclusion">${km('\\Gamma \\;\\vdash\\;')} ${kt('\\mathrm{transp}^i\\bigl(\\mathrm{ua}(e)(i)\\bigr)', 'transp')} ${km('\\,')} ${kt('a', 'a')} ${km('\\;\\equiv\\;')} ${kt('e(a)', 'ea')} ${km('\\;:\\; B')}</div>
      </div>
    `,
    ...
    update: (t: number, _elapsed?: number) => {
      ...
      if (leftCurvePoints.length > 0) {
        const frac = t * (leftCurvePoints.length - 1);
        const i0 = Math.min(Math.floor(frac), leftCurvePoints.length - 2);
        leftDot.position.lerpVectors(leftCurvePoints[i0], leftCurvePoints[i0 + 1], frac - i0);
      }

      // Right side: morphing dot along transport arc, color lerp
      if (rightCurvePoints.length > 0) {
        const frac = t * (rightCurvePoints.length - 1);
        const i0 = Math.min(Math.floor(frac), rightCurvePoints.length - 2);
        morphDot.position.lerpVectors(rightCurvePoints[i0], rightCurvePoints[i0 + 1], frac - i0);
        const cA = new THREE.Color(0x44DDFF);
        const cB = new THREE.Color(0xFF8844);
