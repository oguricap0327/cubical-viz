<script lang="ts">
  ...
  import PartialElementsRule from './lib/rules/PartialElementsRule.svelte';
  import ReflRule from './lib/rules/ReflRule.svelte';
  import FunExtRule from './lib/rules/FunExtRule.svelte';
  import HCompRule from './lib/rules/HCompRule.svelte';
  import PiSigmaCompRule from './lib/rules/PiSigmaCompRule.svelte';
  ...
  let isRuleBased = $derived(
    ...
    current === 'partial-elements-rule' ||
    current === 'refl-rule' ||
    current === 'fun-ext-rule' ||
    current === 'hcomp-rule' ||
    current === 'pi-sigma-comp'
  );
    ...
    {:else if isRuleBased}
      {#key current}
        ...
        {:else if current === 'path-inv-rule'}
          <PathInvRule />
        {:else if current === 'partial-elements-rule'}
          <PartialElementsRule />
        {:else if current === 'refl-rule'}
          <ReflRule />
        {:else if current === 'fun-ext-rule'}
          <FunExtRule />
        {:else if current === 'hcomp-rule'}
          <HCompRule />
        {:else if current === 'pi-sigma-comp'}
          <PiSigmaCompRule />
        {/if}
      {/key}
    {:else}
